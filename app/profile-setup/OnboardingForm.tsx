...
export default function UserOnboardingForm() {
      ...
      try {
        ...
        const docSnap = await getDoc(docRef);
          ...
          setIsRedirecting(true)
            router.replace("/")
            {!isRedirecting && toast.success(`Welcome Back, ${docSnap.data().firstName}`)};
        }
        if (docSnap.exists()) {
  ...
  return (
      <form ref={formRef} onSubmit={handleSubmit(onSubmit)} className="w-full h-full bg-white/10 backdrop-blur-lg">
            {/* Step Dots */}
            ...
            <AnimatePresence mode="wait">
              ...
              >
                {/* Step 1: About You */}
                {step === 1 && <Step1AboutYou {...{register,control,watch,errors,dirtyFields,setValue}}/> }

                {/* Step 2: Interests */}
                {step === 2 && <Step2Interests {...{register,control,watch,errors}}/>}

                {/* Step 3: Experience */}
                {step === 3 && <Step3Experience {...{register,control,errors,dirtyFields}}/>}
              </motion.div>
            </AnimatePresence>
